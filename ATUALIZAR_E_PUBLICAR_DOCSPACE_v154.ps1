...
if (-not (Test-Path $BackendProjeto)) { Falhar "A pasta backend-worker não foi encontrada." }

$Zip = Get-ChildItem -Path @(
    (Join-Path $env:USERPROFILE "Downloads\DocSpace-GitHub-v154*.zip"),
    (Join-Path $Projeto "DocSpace-GitHub-v154*.zip")
) -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1

if (-not $Zip) { Falhar "Baixe DocSpace-GitHub-v154.zip e deixe-o em Downloads ou na pasta do projeto." }

$Extracao = Join-Path $env:TEMP "docspace-atualizacao-v154"
if (Test-Path $Extracao) { Remove-Item $Extracao -Recurse -Force -ErrorAction SilentlyContinue }

Write-Host "`n=== EXTRAINDO A VERSÃO 154 ===" -ForegroundColor Cyan
Expand-Archive -LiteralPath $Zip.FullName -DestinationPath $Extracao -Force

$Fonte = Join-Path $Extracao "DocSpace-Static-GitHub-v154"
if (-not (Test-Path (Join-Path $Fonte "frontend\index.html"))) { Falhar "O pacote v154 está incompleto." }
if (-not (Test-Path (Join-Path $Fonte "backend-worker\src\worker.js"))) { Falhar "O Worker v154 não foi encontrado no pacote." }

Write-Host "`n=== INSTALANDO FRONTEND E WORKER ===" -ForegroundColor Cyan
...
Copy-Item (Join-Path $Fonte "ALTERACOES_v154.md") (Join-Path $Projeto "ALTERACOES_v154.md") -Force

Write-Host "`n=== VALIDANDO A CORREÇÃO ===" -ForegroundColor Cyan
...
$ConteudoFrontend = Get-Content -Raw (Join-Path $FrontendProjeto "script.js")
$ConteudoWorker = Get-Content -Raw (Join-Path $BackendProjeto "src\worker.js")
if ($ConteudoFrontend -notmatch "renderOfficeAiDialog") { Falhar "O Assistente Office contextual não foi instalado." }
if ($ConteudoFrontend -notmatch "office-sticky-controls") { Falhar "A barra fixa do Office não foi instalada." }
if ($ConteudoWorker -notmatch '"office-word"' -or $ConteudoWorker -notmatch '"office-excel"') { Falhar "As ações de IA do Office não foram instaladas no Worker." }

Write-Host "`n=== ENVIANDO A V154 PARA A MAIN ===" -ForegroundColor Cyan
...
if ($Alteracoes) {
    git commit -m "DocSpace v154 - Office fixo e IA contextual"
    Verificar-Saida "Não foi possível criar o commit da v154."
} else {
...
$Publicacao = Join-Path $env:TEMP "docspace-gh-pages-v154"
...
$ConfigPages = Join-Path $env:TEMP "docspace-pages-v154.json"
...
Write-Host "`n=== PUBLICANDO O WORKER COM A IA DO OFFICE ===" -ForegroundColor Cyan
...
Pop-Location
if ($ResultadoWorker -ne 0) { Falhar "O site foi atualizado, mas o Cloudflare Worker não foi publicado." }
...
if ($Publicado) {
    Write-Host "DOCSPACE V154 PUBLICADO COM SUCESSO" -ForegroundColor Green
} else {
...
Write-Host "Correções: topo administrativo + barras Office fixas" -ForegroundColor Green
Write-Host "IA: Assistente Word e Assistente Excel contextuais" -ForegroundColor Green

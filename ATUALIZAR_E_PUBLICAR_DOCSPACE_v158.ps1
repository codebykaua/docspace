...
$Zip = Get-ChildItem -Path @(
    (Join-Path $env:USERPROFILE "Downloads\DocSpace-GitHub-v158*.zip"),
    (Join-Path $Projeto "DocSpace-GitHub-v158*.zip")
) -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1

if (-not $Zip) { Falhar "Baixe DocSpace-GitHub-v158.zip e deixe-o em Downloads ou na pasta do projeto." }

$Extracao = Join-Path $env:TEMP "docspace-atualizacao-v158"
if (Test-Path $Extracao) { Remove-Item $Extracao -Recurse -Force -ErrorAction SilentlyContinue }

Write-Host "`n=== EXTRAINDO A VERSÃO 158 ===" -ForegroundColor Cyan
Expand-Archive -LiteralPath $Zip.FullName -DestinationPath $Extracao -Force

$Fonte = Join-Path $Extracao "DocSpace-Static-GitHub-v158"
if (-not (Test-Path (Join-Path $Fonte "frontend\index.html"))) { Falhar "O pacote v158 está incompleto." }
if (-not (Test-Path (Join-Path $Fonte "backend-worker\src\worker.js"))) { Falhar "O Worker v158 não foi encontrado." }

Write-Host "`n=== INSTALANDO A V158 COM CORREÇÕES DE INTERFACE E OFFICE ===" -ForegroundColor Cyan
...
Copy-Item (Join-Path $Fonte "ALTERACOES_v158.md") (Join-Path $Projeto "ALTERACOES_v158.md") -Force
...
if ($Css -notmatch "DocSpace v158" -or $Css -notmatch "pdf-tool-grid" -or $Css -notmatch "sidebar-width: 184px") { Falhar "As correções visuais da v158 não foram instaladas." }
if ($Script -notmatch "data-word-insert-date" -or $Script -notmatch "data-excel-formula" -or $Script -notmatch "data-excel-freeze-header") { Falhar "As novas ferramentas do Word e Excel não foram instaladas." }
...
Write-Host "`n=== ENVIANDO A V158 PARA A MAIN ===" -ForegroundColor Cyan
...
if ($Alteracoes) {
    git commit -m "DocSpace v158 - Interface corrigida e Office mais completo"
    Verificar-Saida "Não foi possível criar o commit da v158."
} else {
...
$Publicacao = Join-Path $env:TEMP "docspace-gh-pages-v158"
...
git checkout --orphan docspace-publicacao-v158
...
git add -A
git -c user.name="DocSpace Publisher" -c user.email="codebykaua@users.noreply.github.com" commit -m "Publica DocSpace v158"
if ($LASTEXITCODE -ne 0) {
    Pop-Location
...
$ConfigPages = Join-Path $env:TEMP "docspace-pages-v158.json"
...
Write-Host "`n=== PUBLICANDO A IA DO OFFICE NO CLOUDFLARE ===" -ForegroundColor Cyan
...
for ($i = 1; $i -le 36; $i++) {
    ...
    try {
        ...
        if ($Resposta.StatusCode -eq 200 -and $Resposta.Content -match "cache-reset-v158") {
            $Publicado = $true
            break
        }
    } catch {
...
if ($Publicado) {
    Write-Host "DOCSPACE V158 PUBLICADO COM SUCESSO" -ForegroundColor Green
} else {
...
Write-Host "PDF e sidebar: caixas compactas, alinhadas e sem cortes" -ForegroundColor Green
Write-Host "Office: Word, Excel e PowerPoint com IA contextual aprimorada" -ForegroundColor Green

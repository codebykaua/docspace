...
$Zip = Get-ChildItem -Path @(
    (Join-Path $env:USERPROFILE "Downloads\DocSpace-GitHub-v159*.zip"),
    (Join-Path $Projeto "DocSpace-GitHub-v159*.zip")
) -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1

if (-not $Zip) { Falhar "Baixe DocSpace-GitHub-v159.zip e deixe-o em Downloads ou na pasta do projeto." }

$Extracao = Join-Path $env:TEMP "docspace-atualizacao-v159"
if (Test-Path $Extracao) { Remove-Item $Extracao -Recurse -Force -ErrorAction SilentlyContinue }

Write-Host "`n=== EXTRAINDO A VERSÃO 159 ===" -ForegroundColor Cyan
Expand-Archive -LiteralPath $Zip.FullName -DestinationPath $Extracao -Force

$Fonte = Join-Path $Extracao "DocSpace-Static-GitHub-v159"
if (-not (Test-Path (Join-Path $Fonte "frontend\index.html"))) { Falhar "O pacote v159 está incompleto." }

Write-Host "`n=== INSTALANDO A CORREÇÃO RESPONSIVA ===" -ForegroundColor Cyan
...
Copy-Item (Join-Path $Fonte "ALTERACOES_v159.md") (Join-Path $Projeto "ALTERACOES_v159.md") -Force
...
if ($Css -notmatch "DocSpace v159" -or $Css -notmatch "Anula o menu branco legado" -or $Css -notmatch "max-width: 1100px") {
    Falhar "A correção responsiva da v159 não foi instalada."
}
if ($Index -notmatch "cache-reset-v159" -or $Index -notmatch "lovable-original.css\?v=159") {
    Falhar "Os arquivos de cache da v159 não foram instalados."
}

Write-Host "`n=== ENVIANDO A V159 PARA A MAIN ===" -ForegroundColor Cyan
...
if ($Alteracoes) {
    git commit -m "DocSpace v159 - Responsividade real em tablets e celulares"
    Verificar-Saida "Não foi possível criar o commit da v159."
} else {
...
$Publicacao = Join-Path $env:TEMP "docspace-gh-pages-v159"
...
git checkout --orphan docspace-publicacao-v159
...
git add -A
git -c user.name="DocSpace Publisher" -c user.email="codebykaua@users.noreply.github.com" commit -m "Publica DocSpace v159"
if ($LASTEXITCODE -ne 0) {
    Pop-Location
...
$ConfigPages = Join-Path $env:TEMP "docspace-pages-v159.json"
...
for ($i = 1; $i -le 36; $i++) {
    ...
    try {
        ...
        if ($Resposta.StatusCode -eq 200 -and $Resposta.Content -match "cache-reset-v159") {
            $Publicado = $true
            break
        }
    } catch {
...
if ($Publicado) {
    Write-Host "DOCSPACE V159 PUBLICADO COM SUCESSO" -ForegroundColor Green
} else {
...
Write-Host "Responsividade: sidebar em drawer, sem menu branco sobreposto" -ForegroundColor Green
Write-Host "PDF, Administração, Atendimento e Office adaptados" -ForegroundColor Green
